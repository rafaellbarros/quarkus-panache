...
class PessoaServiceTest {
    ...
    void setUp() {
        ...
        BDDMockito.when(pessoaMapperMock.toDTO(pessoaRepositoryMock.findAll().list())).thenReturn(singletonList(pessoaDTOMock));

        BDDMockito.when(pessoaMapperMock.toDTO(pessoaMock)).thenReturn(pessoaDTOMock);

        BDDMockito.when(pessoaRepositoryMock.findById(ArgumentMatchers.anyLong())).thenReturn(pessoaMock);

        BDDMockito.doNothing().when(pessoaRepositoryMock).persist(ArgumentMatchers.any(Pessoa.class));
